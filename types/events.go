...
// ABCIEvents returns all stored Event objects as avsi.Event objects.
func (em EventManager) AVSIEvents() []avsi.Event {
	return em.events.ToAVSIEvents()
}
...
func TypedEventToEvent(tev proto.Message) (Event, error) {
	...
	for _, k := range keys {
		v := attrMap[k]
		attrs = append(attrs, avsi.EventAttribute{
			Key:   k,
			Value: string(v),
		})
	}
...
// ParseTypedEvent converts avsi.Event back to a typed event.
func ParseTypedEvent(event avsi.Event) (proto.Message, error) {
...
// ToAVSIEvents converts a slice of Event objects to a slice of avsi.Event
// objects.
func (e Events) ToAVSIEvents() []avsi.Event {
	res := make([]avsi.Event, len(e))
	for i, ev := range e {
		res[i] = avsi.Event{Type: ev.Type, Attributes: ev.Attributes}
	}
...
func StringifyEvent(e avsi.Event) StringEvent {
	res := StringEvent{EventType: e.Type}
...
// objects.
func StringifyEvents(events []avsi.Event) StringEvents {
...
func MarkEventsToIndex(events []avsi.Event, indexSet map[string]struct{}) []avsi.Event {
	indexAll := len(indexSet) == 0
	updatedEvents := make([]avsi.Event, len(events))

	for i, e := range events {
		updatedEvent := avsi.Event{
			Type:       e.Type,
			Attributes: make([]avsi.EventAttribute, len(e.Attributes)),
		}

		for j, attr := range e.Attributes {
			_, index := indexSet[fmt.Sprintf("%s.%s", e.Type, attr.Key)]
			updatedAttr := avsi.EventAttribute{
				Key:   attr.Key,
				Value: attr.Value,
