...
// AVSIError will resolve an error code/log from an abci result into
...
func AVSIError(codespace string, code uint32, log string) error {
	if e := getUsed(codespace, code); e != nil {
...
//
// If the wrapped error does not provide AVSICode method (ie. stdlib errors),
