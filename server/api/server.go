...
type Server struct {
	...
	GRPCSrv           *grpc.Server
	logger            log.Logger
	metrics           *telemetry.Metrics //nolint:nolintlint
...
func New(clientCtx client.Context, logger log.Logger, grpcSrv *grpc.Server) *Server {
	...
	return &Server{
		...
		GRPCGatewayRouter: runtime.NewServeMux(
			// Squared marshaler option is required for gogo proto
			...
			// Squared header matcher for mapping request headers to
			// GRPC metadata
...
// Start starts the API server. Internally, the API server leverages PellDVS's
// JSON RPC server. Configuration options are provided via config.APIConfig
// and are delegated to the PellDVS JSON RPC server.
//
